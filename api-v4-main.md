...
POST [https://api.eye-square.com/v4/graphql](https://api.eye-square.com/v4/graphql)

## The Graphiql explorer

Best starting point for using the api is the [interactive explorer](/docs/explorer). Through it you can easily make api calls and have an interactive feedback for doing so.
...
![falsy error](/images/docs/falsy-explorer-error.png)
...
POST [https://api.eye-square.com/v4/auth](https://api.eye-square.com/v4/auth)

When authentication of the user was successful you will recieve a user profile including a freshly generated token.

Example request using curl

`curl -X POST -H 'Content-Type: application/json' -d '{ "email": "users@email", "password": "superSecretPassword" }' https://api.eye-square.com/v4/auth`
...
Example token request ([view in explorer](/docs/explorer?query=%7B%0A++me+%7B%0A++++...+on+User+%7B%0A++++++name%0A++++%7D%0A++%7D%0A%7D%0A&variables=)):

`curl -X POST -H 'Content-Type: application/json' -d '{ "query": "{ me { ...on User { name } }}" }' -H 'Authorization: Bearer your.access.token' https://api.eye-square.com/v4/graphql`
...
[explorer link](/docs/explorer?query=%7B%0A++organizationTeam+%7B%0A++++name%0A++++apiSecret%0A++%7D%0A%7D%0A&variables=)

To make an api secret based call, just include the secret as a query param:

`https://api.eye-square.com/v4/graphql?apiSecret=yourSecret`

Example call from above:

`curl -X POST -H 'Content-Type: application/json' -d '{ "query": "{ me { ...on Team { name } }}" }' -H https://api.eye-square.com/v4/graphql?apiSecret=yourSecret`
...
[`{ "query": "{ me { role }}" }`](/docs/explorer?query=%7B%0A++me+%7B%0A++++role%0A++%7D%0A%7D%0A&variables=)
...
At the same time the response Object contains a lot of useful information for pagination through the list, it's best explained with an actual query ([view in explorer](/docs/explorer?query=%7B%0A++projects%28last%3A+2%29+%7B+%23+get+the+last+2+projects%0A++++pageInfo+%7B%0A++++++startCursor+%23+cursor+of+first+recieved+edge%0A++++++endCursor+%23+cursor+of+last+recieved+edge%0A++++++hasNextPage+%23+true+if+there+is+content+after+the+last+recieved+edge+%0A++++++hasPreviousPage+%23+true+if+there+is+content+before+the+first+recieved+edge%0A++++%7D%0A++++totalCount+%23+total+number+of+objects+you+can+recieve%0A++++nodes+%7B+%23+shortcut+to+nodes+in+case+you+don%27t+need+a+pagination+cursor%0A++++++...project%0A++++%7D%0A++++edges+%7B%0A++++++node+%7B+%23+the+content+itself%0A++++++++...project%0A++++++%7D%0A++++++cursor+%23+cursor+of+the+edge%2C+only+used+in+pagination+params%0A++++%7D%0A++%7D%0A%7D%0A%0Afragment+project+on+Project+%7B%0A++id%0A%7D&variables=)):
...
For example if you want only retrieve the last 10 projects ([view in explorer](/docs/explorer?query=%7B%0A++projects%28last%3A+10%29+%7B%0A++++nodes+%7B%0A++++++...project%0A++++%7D%0A++%7D%0A%7D%0A%0Afragment+project+on+Project+%7B%0A++id%0A++description%0A%7D%0A&variables=)):
...
For all those types we export fitting graphql types in our schema though, so you know which contextType requires which fields ([explorer](/docs/explorer?query=%7B%0A++__type%28name%3A+%22ContextTaskDataFacebookNewsfeedStatic%22%29+%7B%0A++++fields+%7B%0A++++++name%0A++++++description%0A++++%7D%0A++%7D%0A%7D%0A&variables=)):
...
![task flow](/images/docs/task-flow.jpg)
...
![context task flow](/images/docs/context-task-flow.jpg)

### Supported context types

The following types are currently API supported:

[view in explorer](/docs/explorer?query=%7B%0A++__type%28name%3A+%22CONTEXT_TASK_TYPES%22%29+%7B%0A++++enumValues+%7B%0A++++++name%0A++++++description%0A++++%7D%0A++%7D%0A%7D%0A&variables=)

Additional information about the ad formats can be found here: [http://e2.ms/adsupport](http://e2.ms/adsupport)

Each Context type has different setup params. If you want to know which data is required for setup you can ask our graphql schema by searching for "ContextTaskData<Ad-Type>" Where ad type is one of the types you retrieved above, for example [ContextTaskDataFacebookNewsfeedVideoAutoplay](/docs/explorer?query=%7B%0A++__type%28name%3A+%22ContextTaskDataFacebookNewsfeedVideoAutoplay%22%29+%7B%0A++++name%0A++++description%0A++++fields+%7B%0A++++++name%0A++++++description%0A++++%7D%0A++%7D%0A%7D%0A&variables=)
...
BaseUrl: https://recording-scheduler.eye-square.com/scheduler/entry/
...
![tv flow](/images/docs/tv-flow.jpg)
...
[https://recording-scheduler.eye-square.com/scheduler/entry](https://recording-scheduler.eye-square.com/scheduler/entry)
...
[https://recording-scheduler.eye-square.com/scheduler/entry?SessionID=](https://recording-scheduler.eye-square.com/scheduler/entry?SessionID=)
...
| ------------- | ------------ | ---------------------------- | ---------------------------- | -------------- | ------------------ |
| static/video  | true/false   | Facebook, Instagram, Twitter | task                         | "loaded": true | -                  |
| static        | true         | Facebook                     | task-task / task-survey-task | -              | "visible":10000    |
| static        | true         | Instagram, Twitter           | task-task / task-survey-task | -              | "visible":1000     |
| static        | false        | Facebook, Instagram, Twitter | task-task / task-survey-task | -              | "visible":1000     |
| video         | true         | Facebook                     | task-task / task-survey-task | -              | "percentPlayed":90 |
| video         | true         | Instagram, Twitter, Snapchat | task-task / task-survey-task | -              | "percentPlayed":50 |
| video         | false        | Facebook, Instagram, Twitter | task-task / task-survey-task | -              | "played":500       |
| video         | false        | Snapchat                     | task-task / task-survey-task | -              | "percentPlayed":50 |
| preroll video | true         | YouTube                      | task                         | "played":5000  | -                  |
| preroll video | false        | YouTube                      | task                         | "played":500   | -                  |
| preroll video | true         | YouTube                      | task-task / task-survey-task | -              | "played":5000      |
| preroll video | false        | YouTube                      | task-task / task-survey-task | -              | "played":500       |
